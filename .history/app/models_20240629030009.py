...
for i in range(len(app.controllers.numberannoument) ):
  ...
  #  print(app.controllers.numeroLugares[i][1].text)
     dataanuncio.append((app.controllers.marcaCarro[i][0].text,int(app.controllers.numeroLugares[i][0].text),app.controllers.taxasextras[i][0].text,int app.controllers.preco[i][0].text,app.controllers.numeroLugares[i][1].text  ))
